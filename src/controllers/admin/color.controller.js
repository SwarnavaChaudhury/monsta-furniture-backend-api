...
exports.create = async (request, response) => {
    ...
    try {
        ...
        const output = {
            _status: false,
            _message: 'Color validation failed!',
            _data: null
        }
        response.send(output);
    }
...
exports.view = async (request, response) => {
    ...
    let totalRecords = await colorModel.find(filter).countDocuments();
...
exports.details = async (request, response) => {
    ...
    await colorModel.findById(request.body.id)
        .then((result) => {
            if (result) {
                const output = {
                    _status: true,
...
exports.changeStatus = async (request, response) => {
    try {
        ...
        if (!colors.length) {
            return response.send({
                _status: false,
                _message: 'No matching colors found',
                _data: null
            });
        }
        ...
        return response.send({
            ...
            _data: colors.map(clr => ({ id: clr._id, previous_status: clr.status, new_status: !clr.status }))
        });
