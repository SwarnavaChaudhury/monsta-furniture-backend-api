...
exports.create = async (request, response) => {
    ...
    try {
        ...
        const output = {
            _status: false,
            _message: 'Sub-Category validation failed!',
            _data: null
        }
        response.send(output);
    }
...
exports.view = async (request, response) => {
    ...
    let totalRecords = await subCategoryModel.find(filter).countDocuments();
        ...
        .then((result) => {
            if (result.length > 0) {
                const output = {
                    ...
                    _image_path: process.env.sub_category_image,
                    _data: result
                }
                response.send(output);
            } else {
...
exports.details = async (request, response) => {
    ...
    await subCategoryModel.findById(request.body.id)
        .then((result) => {
            if (result) {
                const output = {
                    _status: true,
